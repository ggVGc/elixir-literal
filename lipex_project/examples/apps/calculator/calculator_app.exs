...
defmodule CalculatorApp do
  ...
  # Define mathematical functions using Lipex
  deflipex ~~((def square (x) (* x x)))
  deflipex ~~((def cube (x) (* x x x)))
  deflipex ~~((def abs_val (x) (if (< x 0) (* x -1) x)))
...
# Start the calculator if this file is run directly
if String.contains?(to_string(:erlang.get(:progname)), "calculator_app.exs") do
  CalculatorApp.start()
end