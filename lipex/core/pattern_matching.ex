defmodule Lipex.Core.PatternMatching do
  ...
  defp convert_map_pattern_args(args) do
    ...
    |> Enum.map(fn
      [key, value_pattern] ->
        elixir_key = case convert_lipex_pattern_to_elixir(key) do
          atom when is_atom(atom) -> atom
          other -> other
        end
    ...
    case find_pipe_operator(elements) do
      ...
      pipe_index ->
        ...
        case tail_elements do
          ...
          _ -> raise "Cons pattern can only have one element after |"
        end
    end
  end
